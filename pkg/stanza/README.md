...
The performance impact does not occur when using receivers based on Stanza inputs that support batching with no operators. Currently these are: File Log receiver.
...
The performance impact may be also observed when using any receiver (batching or non-batching) with additional operators configured.
...
the next operator in Stanza pipeline will split those batches into single entries.
